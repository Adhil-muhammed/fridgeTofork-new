...
const BottomNavigationBar: React.FC<BottomNavigationBarProps> = ({ activeTab, setActiveTab, setIsChatOverlayOpen }) => {
  ...
  const handleChatFabClick = async () => {
    if (chatContext?.isLiveSessionActive) {
      ...
      // Automatically start live session when chat is opened from FAB
